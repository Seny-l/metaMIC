...
def parseargs():
    ...
    parser.add_argument("--pileup",default='pileup.out',help="path to pileup file")
    ...
    parser.add_argument("--contig",default='assemblies',help='path to assemblies')
    ...
    if not os.path.getsize(args.pileup):                                                       
        if os.path.getsize(args.output+"/temp/pileup/contigs_pipelup.out"):
            ...
            if not os.path.exists(args.contig):
                if os.path.exists(args.output+"/temp/contig/filtered_contigs.fa"):
                    args.contig = args.output+"/temp/contig/filtered_contigs.fa"
                else:
                    print("Can not find assemblies: "+ args.contig)
                    exit(-1)                                                                             
            os.system("mkdir -p " + args.output+"/temp/pileup")
            pileup_command=' '.join([args.samtools,'mpileup -C 50 -A -f',args.contig,args.bam," | awk","'","$3 !=","\"N\"","'",">",args.output+"/temp/pileup/contigs_pipelup.out"])  
            args.pileup=args.output+"/temp/pileup/contigs_pipelup.out"                   
...
def main():
    ...
    if not os.path.isdir(args.output+"/temp/pileup"):
        os.system("mkdir -p "+ args.output+"/temp/pileup")  
